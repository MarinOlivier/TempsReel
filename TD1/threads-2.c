...
int main() {
  ...
  thread_args amazingArgs;
  amazingArgs.count = 2;
  amazingArgs.args = malloc(amazingArgs.count*sizeof(int));
  amazingArgs.args[0] = 42;
  amazingArgs.args[1] = 45;

  err = pthread_create(&tid, &attr, (void *) thread_fils, &amazingArgs);
  if (err) {
